...
- [22. Typical misuses and antipatterns](#22-typical-misuses-and-antipatterns)
...
## 22. Typical misuses and antipatterns

The pitfalls below are the failures that most often turn a generator into a
correctness bug or an IDE performance problem. They are collected from the
Roslyn design document and cookbook
(<https://github.com/dotnet/roslyn/blob/main/docs/features/incremental-generators.md>,
<https://github.com/dotnet/roslyn/blob/main/docs/features/incremental-generators.cookbook.md>)
and Andrew Lock's performance/cacheability articles
(<https://andrewlock.net/creating-a-source-generator-part-9-avoiding-performance-pitfalls-in-incremental-generators/>,
<https://andrewlock.net/creating-a-source-generator-part-10-testing-your-incremental-generator-pipeline-outputs-are-cacheable/>).

### Pipeline and incrementality

- **Putting `ISymbol`, `Compilation`, `SemanticModel`, `SyntaxNode`, or
  `Location` in pipeline model values.** These types do not implement value
  equality and pin large object graphs, so caching breaks and memory balloons.
  Extract metadata names, display strings, and primitives into a
  `readonly record struct` as early as possible.
- **Using `ImmutableArray<T>` (or any collection) directly in an equatable
  model without a comparer.** Default equality is by reference, so every run
  looks "changed." Wrap collections in an `EquatableArray<T>` or apply
  `WithComparer`.
- **Calling `Collect()` too early.** Collapsing a many-values provider into one
  array means a single changed item invalidates the whole batch. Filter and
  reduce first; collect only when aggregation is genuinely required.
- **`Combine`-ing with `CompilationProvider` (or another large value) up
  front.** This pairs every input with the whole compilation, defeating
  incrementality. Extract the few stable facts you need from the compilation,
  then combine those.
- **Doing real work in `Initialize` instead of in pipeline callbacks.**
  `Initialize` should only *register* the pipeline; generation work belongs in
  `Select`/`RegisterSourceOutput` so the host can cache and re-run it.

### State and determinism

- **Storing per-compilation state in instance or static fields.** The generator
  instance is reused across compilations and target frameworks and steps can run
  concurrently. All state must flow through pipeline values.
- **Emitting non-deterministic output** (timestamps, GUIDs, `DateTime.Now`,
  machine paths, unordered dictionary/`HashSet` enumeration). This breaks
  caching, reproducible builds, and source-control diffing. Sort and normalize.
- **Caching data globally across target frameworks.** Each TFM inner build is a
  separate compilation with different symbols and preprocessor state; never
  share parsed configuration or symbols across them.

### Scope and semantics

- **Doing expensive semantic work in a `CreateSyntaxProvider` predicate.**
  Predicates run for huge numbers of nodes on every keystroke and must stay
  purely syntactic and cheap; defer semantics to the transform.
- **Using `CreateSyntaxProvider` when `ForAttributeWithMetadataName` would
  work.** The attribute-indexed path is dramatically faster; prefer it for any
  attribute-driven generator.
- **Broad semantic scans** such as "find every type that implements this
  interface anywhere." These cannot be made incremental and dominate IDE time.
- **Depending on another generator's output or on generator execution order.**
  All generators see the same input compilation, run unordered, and never see
  each other's generated source.

### Output and packaging

- **Building `SyntaxNode`s and calling `NormalizeWhitespace()` for final
  output.** This is slow and allocation-heavy; emit text with a writer or
  `StringBuilder`.
- **Throwing from the pipeline to signal user errors.** A thrown exception
  surfaces as `CS8785` and discards the generator's output for that
  compilation. Report a diagnostic (or use a companion analyzer) instead.
- **Forgetting `// <auto-generated/>`**, non-unique hint names, or unqualified
  type names in generated code. Always emit the header, deterministic `.g.cs`
  hint names, and `global::`-qualified type references.
- **Shipping the generator under `lib/` or leaking Roslyn authoring packages.**
  Pack under `analyzers/dotnet/cs` with `IncludeBuildOutput=false` and
  `PrivateAssets="all"` on Roslyn references, or every consumer inherits them.
- **Compiling against a newer Roslyn than the hosts you support.** The
  referenced `Microsoft.CodeAnalysis` version is your minimum host version;
  mismatches surface as `CS9057`/`CS8032`. Pin to the lowest minor that exposes
  the APIs you use.
