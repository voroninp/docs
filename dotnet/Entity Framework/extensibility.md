...
| **IIdentityResolutionInterceptor** | Resolves conflicts when tracking multiple instances with the same key. | No |
...
| **IInstantiationBindingInterceptor** | Intercepts constructor/parameter binding used for entity/materialization instantiation. | **Yes** |
...
Multiple interceptors of the same type can be registered via `.AddInterceptors(interceptor1, interceptor2, ...)`. They execute in registration order.

> **Important for singleton interceptors**: `IMaterializationInterceptor`, `IQueryExpressionInterceptor`, and `IInstantiationBindingInterceptor` are
> **singleton interceptors** shared across all contexts and threads. Never store mutable state in their fields. See [Thread Safety](#thread-safety) in
> Section 12 for detailed guidance.
...
For stateless interceptors, registering once as a singleton avoids repeated allocation. However, **`ISaveChangesInterceptor` is not a singleton
interceptor** and does not contribute to the `ManyServiceProvidersCreatedWarning` concern described in [Section 12](#12-gotchas-and-antipatterns). That
warning applies only to **singleton interceptors** that implement `ISingletonInterceptor`, such as `IQueryExpressionInterceptor`,
`IMaterializationInterceptor`, or `IInstantiationBindingInterceptor`. You may register `SaveChangesInterceptor` as a singleton for efficiency, or as
scoped/transient depending on its dependencies; the service-provider cache is unaffected either way:
...
The finalized model is cached and reused (see [Model Caching and Multiple Models](#model-caching-and-multiple-models)). 

The model is **structurally immutable** — entity types, properties, keys, and relationships cannot be added or removed after finalization. However,
**runtime annotations** (non-serialized metadata) can be added after finalization through `IModelRuntimeInitializer.Initialize()`. Any structural
configuration must happen during the model-building phase (conventions, `OnModelCreating`, or `IModelCustomizer`).
...
5.  EF finalizes the model, then caches it under the current `IModelCacheKeyFactory` key.
    ...
    : RelationalModelCustomizer(dependencies)
{
    public override void Customize(ModelBuilder modelBuilder, DbContext context)
    {
        ...
        foreach (var entityType in modelBuilder.Model.GetEntityTypes().ToList())
        {
            ...
            // Example: add a shadow audit column to every root entity type.
...
// In IDbContextOptionsExtension.ApplyServices (EF Core registers IModelCustomizer as singleton):
services.Replace(ServiceDescriptor.Singleton<IModelCustomizer, MyModelCustomizer>());
```
...
> **Note:** A cached `IModel` consumes 1–10 MB depending on entity count and property complexity. Caching tens to low hundreds of models (for
> feature toggles, schemas, or bounded contexts) is efficient and safe. Beyond ~100–200 cached models, memory consumption becomes significant; for
> large-scale SaaS with one model per tenant, consider external schema management instead. Design-time migrations also usually assume one model per
> context, so multi-model setups may require external tooling.
...
1. **Standard (Design-Time) Annotations**: Added via `HasAnnotation` or `AddAnnotation`. These are part of the core model, serialized into compiled
   models (AOT), evaluated during design-time, and surfaced through relational annotation providers during schema generation. Use these
   ...
   **What "serialized into compiled models" means:** `dotnet ef dbcontext optimize` generates C# source files that reproduce the entire model without
   ...
   by EF Core's runtime annotation code generators (`CSharpRuntimeAnnotationCodeGenerator` and relational/provider variants) that can produce the
   corresponding literal expression. Without generator support for a custom annotation value, generation can fail or omit that value. Non-representable
   values — compiled delegates, service instances, or complex object graphs — should generally be kept out of design-time annotations.
...
```
This metadata is then queried internally by relational annotation providers and migrations services, or by your custom `IModelCustomizer` to
...
*   **Registration**: To inject conventions without requiring the user to modify `OnModelCreating`, implement **`IConventionSetPlugin`** and register
    ...
    services.TryAddEnumerable(
        ServiceDescriptor.Scoped<IConventionSetPlugin, MyConventionPlugin>());
    ```
...
2.  **Manual Bridging**: For application services not bridged automatically, prefer to inject a DI-created interceptor or options object into the
    ...
    `IMaterializationInterceptor`, and `IInstantiationBindingInterceptor`) in the `CoreOptionsExtension` service-provider hash. A new object identity
...
**Why not use `AddScoped` in ASP.NET Core's container?** In the application container, scoped lifetime means *one instance per app scope* (for
example, one HTTP request scope). If multiple `DbContext` instances are created or resolved inside that same app scope, they all receive the same
state-holder instance. State set for one context can therefore affect the others. In contrast, EF Core internal scoped lifetime is *one instance per
`DbContext`*, so each context gets isolated ambient state with no cross-talk.
    ...
    {
        public static async Task<int> SaveChangesWithTagAsync(
    ...
    services.TryAddEnumerable(
        ServiceDescriptor.Scoped<IInterceptor, TaggedSaveInterceptor>());
    ```
...
| :--- | :--- | :--- |
| `IConventionSetPlugin` | Scoped | Register with `TryAddEnumerable`; implementations must be safe for scoped resolution. |
| `IModelCustomizer` | Singleton | Replace with a singleton descriptor to match EF Core's registration. |
...
| Query, method-call, and member translator plugins | Scoped | Use `TryAddEnumerable`; these plugins should not depend on shorter-lived state. |
| `IMigrationsSqlGenerator` / `IMigrationsAnnotationProvider` | Scoped / Singleton | Match EF Core lifetimes when replacing these services. |
| Ordinary interceptors (`IDbCommandInterceptor`, `ISaveChangesInterceptor`, `IIdentityResolutionInterceptor`) | Application-defined | May be per context, scoped, or singleton depending on state and dependencies. |
| Singleton interceptors (`IMaterializationInterceptor`, `IQueryExpressionInterceptor`, `IInstantiationBindingInterceptor`) | Singleton | Shared by EF internal service providers; never store mutable per-context state. |
...
**Expand a custom method/property into standard LINQ** using **`IQueryExpressionInterceptor`** when the helper is only a reusable .NET facade
 over logic EF can already translate. The interceptor substitutes the custom method call or property access with an equivalent LINQ expression
 before EF translates it to SQL—avoiding the need for custom SQL translation.
...
```csharp
services.TryAddEnumerable(ServiceDescriptor.Scoped<IMethodCallTranslatorPlugin, MyMethodTranslatorPlugin>());
```
...
1.  **`IRelationalAnnotationProvider`**: Use this to surface relational model metadata/annotations into the migrations pipeline.
2.  **`IMigrationsAnnotationProvider`**: Use this for annotations related to remove/rename migration diff operations. Prefer inheriting the relational
    base `MigrationsAnnotationProvider` (provider-specific implementations may be internal in some providers).
3.  **`IMigrationsSqlGenerator`**: Inherit the database-specific SQL generator (e.g., `SqlServerMigrationsSqlGenerator`) and override logical
...
```csharp
services.Replace(
    ServiceDescriptor.Scoped<IMigrationsSqlGenerator, MyMigrationsSqlGenerator>());
services.Replace(
...
2.  **Runtime model**: an optimized, trimmed model used while the application runs. Conventions are not re-run, reflection is minimized, and
     runtime-only state (compiled delegates, fast-path lookups) is applied. Specifically trimmed from the runtime model:
     - Design-time-only annotations not needed for queries or saves
     - Convention discovery metadata used only during model building
     - Reflection caches built during the model-building phase
     - Structural information already captured in compiled delegates for fast queries
...
*   **Tooling needs the full model.** Migrations and scaffolding must see all relational schema metadata — tables, columns, keys, check constraints,
    and relational annotations consumed by migrations services. The runtime model intentionally drops much of this for performance, so reusing it for tooling
...
**For application projects**: Provide a class implementing **`IDesignTimeServices`**. EF tooling automatically discovers it from the startup assembly
and calls `ConfigureDesignTimeServices`.

**For provider and library packages**: Auto-discovery from the startup assembly is **not** applied to arbitrary library assemblies. Use explicit
wiring:
- Reference design-time services via `DesignTimeServicesReferenceAttribute` (assembly-level), or
- For providers, use provider-specific design-time discovery such as `DesignTimeProviderServicesAttribute`.

Without explicit wiring, library design-time services may be silently skipped when users run EF tools.
...
*   **Thread the `designTime` flag through.** Honor it in `IModelCacheKeyFactory` (include it in the cache key) and in `IModelRuntimeInitializer`
    ...
    initializer pattern.
*   **Put schema-affecting configuration in standard annotations.** Standard annotations are serialized into compiled models and consumed by
    relational/migrations annotation services; runtime annotations are not. See [Model and Metadata Annotations](#model-and-metadata-annotations).
...
services.AddScoped<IConventionSetPlugin, MyConventionPlugin>();
```
...
*   **Eliminate Runtime Code Generation**: Avoid patterns that require runtime code generation under AOT, such as dynamic IL emission and
    runtime-compiling expression trees. Prefer static method references, source generation, or other precomputed delegates:

    ```csharp
    // Before: runtime-compiles an expression tree.
...
*   **Support Compiled Models**: Ensure your extension is compatible with **Compiled Models** (`dotnet ef dbcontext optimize`), which pre-generate the
    ...
    // In ApplyServices:
    services.Replace(ServiceDescriptor.Singleton<IModelRuntimeInitializer, MyModelRuntimeInitializer>());
    ```
...
*   **Service Provider Bloat**: EF Core caches one internal service provider per unique service-provider-affecting option set. Do not confuse this
    ...
    `IMaterializationInterceptor`, or `IInstantiationBindingInterceptor`. EF's internal `ServiceProviderCache` is keyed by the completed
    ...
    `src/EFCore/DbContextOptions.cs`, and `src/EFCore/Infrastructure/CoreOptionsExtension.cs` in the
    [dotnet/efcore repository](https://github.com/dotnet/efcore).
